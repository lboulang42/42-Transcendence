...
export default class Ball {
	...
	constructor(size_renderer) {
		...
		this.#light = new THREE.PointLight(0xFDA141, 10, 300, 0);
	...
	rect() { // hitbox of the ball for collisions
		return {
	...
	reset(size_renderer) {
		...
		let heading = randomNumberBetween(-Math.PI, Math.PI); // angle of where the ball is heading
		while (Math.abs(heading) > Math.PI /  3 && Math.abs(heading) <  2 * Math.PI /  3) { // makes sure it doesnt go too vertical
			heading = randomNumberBetween(-Math.PI, Math.PI);
		}
		this.direction = {x: Math.cos(heading), y: Math.sin(heading)} // converts the heading to a direction vector
		this.velocity = size_renderer.width / 125; // sets the balls speed proportional to the width of the screen so its fair for everybody
		this.stopBall = true;
	}
	...
	collision(size_renderer) {
		if (this.rect().bottom >= size_renderer.height / 2 && this.direction.y > 0) { // checks if bottom of the ball hits bottom of the map
			this.direction.y *= -1; // goes up
		}
		else if (this.rect().top <= -(size_renderer.height / 2) && this.direction.y < 0) { // checks if top of the ball hits top of the map
			this.direction.y *= -1 // goes down
		}
	}
	...
	draw(scene) {                          	// ensures ball light and trail are rendered and displayed during the game
        scene.add(this.mesh);				// (A scene in Three.js is a container for all objects you want to render.)
